...
| Save (some off) | `[ChildSettings] Saved with {n} notification(s) OFF for {name}: {list}` |
...
| Bulk cancel | `[Notifications] Cancelled {n} OS notifications and removed {m} tracked notifications for child {id}` |
